import { apiChat, type ChatRequestPayload } from './api';
import { decodeBase64Text } from './mediaUtils';
import { estimateCost, resolveBaseUrl } from './store';
import type {
  AIProvider,
  AIProviderConfig,
  ChatMessage,
...
export class ThreadReplyService {
  async requestReply(config: AIProviderConfig, thread: ThreadLane, messages: ChatMessage[]): Promise<ReplyResult> {
    ...
    if (!effective.apiKey.trim() && effective.hasEncryptedApiKey) {
      const apiPayload: ChatRequestPayload = {
...
function formatMessageForOpenAI(message: ChatMessage) {
  ...
  const parts: unknown[] = [];
  if (message.content.text) parts.push({ type: 'text', text: message.content.text });
...
async function requestOpenAiCompatible(config: AIProviderConfig, thread: ThreadLane, messages: ChatMessage[], threadParams?: GenerationParams) {
  ...
  const send = async (includeTemperature: boolean) => {
    ...
    if (!response.ok) {
      return { ok: false as const, text: await response.text() };
    }
  ...
  if (!result.ok && config.kind === 'openai') {
    ...
    if (maybeTempUnsupported) result = await send(false);
  }
  if (!result.ok) throw new Error(result.text || `${config.label} request failed`);
